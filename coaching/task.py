...
def send_session_confirmation(session_id):
    ...
    try:
        ...
        # Email to student
        if session.student.email_notifications:
            send_mail(
                ...
                Session Details: {settings.SITE_URL}/coaching/session/{session.id}/
        ...
        # Email to coach
        if session.coach.user.email_notifications:
            send_mail(
        ...
        for user in [session.coach.user, session.student]:
            if user.email_notifications:
                send_mail(
                    ...
                    Session Link: {settings.SITE_URL}/coaching/session/{session.id}/
                    ''',
                    from_email=settings.DEFAULT_FROM_EMAIL,
        ...
        for user in [session.coach.user, session.student]:
            if user.email_notifications:
                send_mail(
                    ...
                    Join here: {settings.SITE_URL}/coaching/session/{session.id}/
...
def mark_no_shows():
    ...
    count = sessions.update(status='no_show')
    ...
    for session in sessions:
        if session.student.email_notifications:
            send_mail(
                ...
                Leave Review: {settings.SITE_URL}/coaching/session/{session.id}/review/
