...
from tournaments.models import Tournament, Participant, Bracket, Match, MatchDispute, Payment, TournamentShare
import json
...
class MatchDisputeViewTests(TestCase):
	...
	def test_dispute_get_and_post_creates_dispute(self):
		...
		response = self.client.get(url)
		self.assertEqual(response.status_code, 200)
		self.assertIn('File Dispute', response.content.decode('utf-8'))
